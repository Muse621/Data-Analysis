...
### 第八步：分析结论（第 6–7 天）看热力图找出以下问题的答案，每个问题写一条结论：整体次月留存率是多少、哪个月份注册的用户留存最好（颜色最深的行）、留存率在第几个月下降最快、有没有某个月特别异常。然后针对流失最严重的节点提出业务建议，比如：所有 Cohort 的第 1 个月留存率（MonthIndex=1）均在 20%–35% 之间，说明用户在首购后一个月内大量流失。建议在用户首购后第 7 天、第 14 天、第 30 天分别推送个性化召回邮件，目标是将次月留存率提升 5 个百分点。
### 如何验证结果:热力图第 0 列（MonthIndex=0）应全部显示 100.0，如果不是说明计算逻辑有误。UCI 数据集的次月留存率大约在 20%–40% 之间，如果算出来是 90% 或 5% 说明 PERIOD_DIFF 的参数顺序写反了。把热力图发给不懂数据的朋友，让他不加解释地指出"哪个月注册的用户最忠诚"，如果他能答对说明图表表达清晰。
